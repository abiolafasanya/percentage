console.clear()
    ...
    document.addEventListener("submit", (e) => {
        ...
        if (actualScore > obtainableScore) {
            ...
            const pass = color.classList.add('pass').remove('fail')
            const fail = color.classList.add('fail').remove('pass')
            let msgs = evaluate > 79 ? "your score is 80% and above" : "Your score is less than 80%"
            evaluate > 79 ? color.classList.remove('fail').add('pass') : color.classList.remove('pass').add('fail')
